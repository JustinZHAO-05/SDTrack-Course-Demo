...
class coesot(BaseVideoDataset):
    def __init__(self, root=None, image_loader=opencv_loader, split=None):
        ...
        """
        root = env_settings().eotb_dir if root is None else root  # 如果没有提供根路径，则使用环境设置中的默认路径
        super().__init__('coesot', root, image_loader)  # 调用父类的初始化函数
        ...
        if split is not None:
            ...
            if split == 'train':
                file_path = os.path.join(ltr_path, 'data_specs', 'coesot_train_split_skx.txt')
            elif split == 'val':
                file_path = os.path.join(ltr_path, 'data_specs', 'coesot_val_split_skx.txt')
            else:
    ...
    def _get_frame_path(self, seq_name, seq_path, frame_id):
        # 根据采样频率确定帧号
        ...
        # 构建图像路径
        img_path = os.path.join(seq_path, seq_name + '_aps', 'frame{:04}.png'.format(gray_id))
        return img_path
    ...
    def _get_event(self, seq_name, seq_path, frame_id):
        # 获取事件数据，加载三帧图像
        # event1 = os.path.join(seq_path, 'dvs', 'frame{:04}.png'.format(frame_id))
        event1 = os.path.join(seq_path, 'spike_thresh_2.5_decay_0.25', '{:04}.png'.format(frame_id))

        return self.image_loader(event1)  # 返回三个事件图像
        # return self.image_loader(event1, rgb='2_channel_rgb')
    ...
    def get_frames(self, seq_id=None, frame_ids=None, anno=None):
        ...
        event_list = [self._get_event(seq_name, seq_path, f_id) for f_id in frame_ids]  # 获取事件数据
