...
'startFrame': 0},
        ...
        sequence_filter = os.environ.get('SDTRACK_SEQUENCE_FILTER', '').strip()
        if sequence_filter:
            wanted = {name.strip() for name in sequence_filter.split(',') if name.strip()}
            sequence_info_list = [item for item in sequence_info_list if item['name'] in wanted]
