...
def build_SDTrack(cfg, training=True):
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'tiny':
        ...
        hidden_dim = 360            # lite 360    base 768
        patch_start_index = 1
        checkpoint = torch.load(env_settings().pretrained_networks + '/SDTrack-tiny-4x1.pth', map_location="cpu")
        ...
        hidden_dim = 360            # lite 360    base 768
        patch_start_index = 1
        checkpoint = torch.load(env_settings().pretrained_networks + '/SDTrack-tiny-2x2.pth', map_location="cpu")
